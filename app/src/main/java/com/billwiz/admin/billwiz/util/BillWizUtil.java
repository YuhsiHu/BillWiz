...
import com.billwiz.admin.billwiz.database.DBHelper;
...
public class BillWizUtil {
    ...
    public static BillWizRecord backupBillWizRecord;
...
//            R.drawable.sum_header_pie
    ...
    public static void init(Context context) {
        ...
        MY_BLUE = ContextCompat.getColor(BillWizApplication.getAppContext(), R.color.my_blue);
    }

    public static Typeface GetTypeface() {
        if (typefaceLatoLight == null) init(BillWizApplication.getAppContext());
    ...
    public static String GetTodayViewTitle(int fragmentPosition) {
        return BillWizApplication.getAppContext().getString(TODAY_VIEW_TITLE[fragmentPosition]);
    }
    ...
    public static String GetAxisDateName(int type, int position) {
        switch (type) {
            ...
            case Calendar.DAY_OF_WEEK:
                if (WEEK_START_WITH_SUNDAY) return BillWizApplication.getAppContext().getResources()
                        .getString(WEEKDAY_SHORT_START_ON_SUNDAY[position + 1]);
                else return BillWizApplication.getAppContext().getResources()
                        .getString(WEEKDAY_SHORT_START_ON_MONDAY[position + 1]);
            case Calendar.DAY_OF_MONTH:
                return (position + 1) + "";
            case Calendar.MONTH:
                return BillWizApplication.getAppContext().getResources()
    ...
    public static String GetMonthShort(int i) {
        return BillWizApplication.getAppContext().getResources().getString(MONTHS_SHORT[i]);
    }

    public static String GetMonth(int i) {
        return BillWizApplication.getAppContext().getResources().getString(MONTHS[i]);
    }
    ...
    public static int GetTagColor(int tag) {
        return ContextCompat.getColor(BillWizApplication.getAppContext(), TAG_COLOR[tag + 3]);
    }

    public static Drawable GetTagDrawable(int tagId) {
        return ContextCompat.getDrawable(
                BillWizApplication.getAppContext(), TAG_DRAWABLE[tagId + 3]);
    }
    ...
    public static Drawable GetTagIconDrawable(int tagId) {
        return ContextCompat.getDrawable(
                BillWizApplication.getAppContext(), TAG_ICON[tagId + 2]);
    }

    public static String GetTagName(int tagId) {
        return BillWizApplication.getAppContext().getResources().getString(TAG_NAME[tagId + 2]);
    }
    ...
    public static int getStatusBarHeight() {
        int result = 0;
        int resourceId = BillWizApplication.getAppContext()
                .getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = BillWizApplication.getAppContext()
    ...
    public static int dpToPx(int dp) {
        DisplayMetrics displayMetrics = BillWizApplication.getAppContext().getResources().getDisplayMetrics();
    ...
    public static String GetCurrentVersion() {
        return "BillWiz V" + BillWizApplication.VERSION / 100 + "." + BillWizApplication.VERSION % 100 / 10 + "." + BillWizApplication.VERSION % 10;
    }
...
//        if (file.exists()) file.delete();
    ...
    private static BillWizUtil ourInstance = new BillWizUtil();

    public static BillWizUtil getInstance() {
        if (ourInstance == null || typefaceLatoLight == null || typefaceLatoHairline == null) {
            ourInstance = new BillWizUtil();
            init(BillWizApplication.getAppContext());
        }
        return ourInstance;
    }
