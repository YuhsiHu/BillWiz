...
public class MonthViewRecyclerViewAdapter
    ...
    private ArrayList<Map<Integer, ArrayList<BillWizRecord>>> ExpanseList;
    ...
    public MonthViewRecyclerViewAdapter(
        ...
        if (!IS_EMPTY) {
            ...
            Map<Integer, ArrayList<BillWizRecord>> Expanse;
            ...
            // for this month
            dateStringList.add(BillWizUtil.GetMonthShort(nowMonth + 1) + " " + nowYear);
            dateShownStringList.add(" in " + BillWizUtil.MONTHS_SHORT[nowMonth + 1] + " " + nowYear);
            ...
            for (BillWizRecord coCoinRecord : list) {
            ...
            while (!now.after(monthEnd)) {
                Calendar leftWeekRange = BillWizUtil.GetThisWeekLeftRange(now);
                Calendar rightWeekRange = BillWizUtil.GetThisWeekRightRange(now);
                Calendar rightShownWeekRange = BillWizUtil.GetThisWeekRightShownRange(now);
                String dateString = BillWizUtil.GetMonthShort(leftWeekRange.get(Calendar.MONTH) + 1) + " " +
                        leftWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftWeekRange.get(Calendar.YEAR) + " - " +
                        BillWizUtil.GetMonthShort(rightShownWeekRange.get(Calendar.MONTH) + 1) + " " +
                ...
                dateShownStringList.add(" from " +
                        BillWizUtil.GetMonthShort(leftWeekRange.get(Calendar.MONTH) + 1) + " " +
                ...
                now = BillWizUtil.GetNextWeekLeftRange(now);
            }
        }
    }
    ...
    public void onBindViewHolder(final viewHolder holder, final int position) {
        ...
        if (IS_EMPTY) {
            ...
            holder.expanseSum.setText(BillWizUtil.GetInMoney((int) (double) SumList.get(position)));

            holder.date.setTypeface(BillWizUtil.GetTypeface());
            holder.expanseSum.setTypeface(BillWizUtil.typefaceLatoLight);

            holder.tags.setTypeface(BillWizUtil.typefaceLatoLight);
            if ("zh".equals(BillWizUtil.GetLanguage())) {
                holder.tags.setText(" ● " + records + BillWizApplication.getAppContext().getResources().getString(R.string.report_view_records) + tags + BillWizApplication.getAppContext().getResources().getString(R.string.report_view_tags));
            } else {
                holder.tags.setText(" ● " + records + " " + BillWizApplication.getAppContext().getResources().getString(R.string.report_view_records) + " " + tags + " " + BillWizApplication.getAppContext().getResources().getString(R.string.report_view_tags));
            }
            ...
            if (SumList.get(position).equals(Double.valueOf(0))) {
                ...
                holder.emptyTip.setTypeface(BillWizUtil.typefaceLatoLight);
            } else {
                ...
                public void onClick(View v) {
                    if ("zh".equals(BillWizUtil.GetLanguage())) {
                        dialogTitle = mContext.getResources().getString(R.string.in)
                                ...
                                " " + BillWizUtil.GetSpendString((int) (double) SumList.get(position));
                    } else {
                        dialogTitle = BillWizUtil.GetSpendString((int) (double) SumList.get(position)) +
        ...
        public void onValueSelected(int i, SliceValue sliceValue) {
            ...
            Snackbar snackbar =
                    Snackbar
                            ...
                            .margin(15, 15)
                            .backgroundDrawable(BillWizUtil.GetSnackBarBackground(fragmentPosition - 2))
                            .text(text)
                            ...
                            .actionListener(new ActionClickListener() {
                                ...
                                public void onActionClicked(Snackbar snackbar) {
                                    List<BillWizRecord> shownCoCoinRecords
