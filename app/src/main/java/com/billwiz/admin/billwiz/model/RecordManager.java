...
import com.billwiz.admin.billwiz.database.DB;
...
public class RecordManager {
    ...
    public static List<BillWizRecord> SELECTED_RECORDS;

    public static Integer SUM;
    public static List<BillWizRecord> RECORDS;
...
// saveRecord///////////////////////////////////////////////////////////////////////////////////////
    public static long saveRecord(final BillWizRecord billWizRecord) {
        ...
        if (BuildConfig.DEBUG)
            if (BuildConfig.DEBUG) Log.d("BillWiz", "recordManager.saveRecord: Save " + billWizRecord.toString() + " S");
        insertId = db.saveRecord(billWizRecord);
        if (insertId == -1) {
            if (BuildConfig.DEBUG)
                if (BuildConfig.DEBUG) Log.d("BillWiz", "recordManager.saveRecord: Save the above BillWizRecord FAIL!");
            Toasty.error(BillWizApplication.getAppContext(),BillWizApplication.getAppContext().getString(R.string.save_failed_locale)).show();
        } else {
            if (BuildConfig.DEBUG)
                if (BuildConfig.DEBUG) Log.d("BillWiz", "recordManager.saveRecord: Save the above BillWizRecord SUCCESSFULLY!");
            RECORDS.add(billWizRecord);
            SUM += (int) billWizRecord.getMoney();
//            if (user != null) {
//                // already login
...
//            }
            Toasty.success(BillWizApplication.getAppContext(),BillWizApplication.getAppContext().getString(R.string.save_successfully_locale)).show();
        }
        return insertId;
    }
// save tag/////////////////////////////////////////////////////////////////////////////////////////
    public static int saveTag(Tag tag) {
        int insertId = -1;
        if (BuildConfig.DEBUG) {
            if (BuildConfig.DEBUG) Log.d("BillWiz", "recordManager.saveTag: " + tag.toString());
        }
        ...
        if (insertId == -1) {
            if (BuildConfig.DEBUG) {
                if (BuildConfig.DEBUG) Log.d("BillWiz", "Save the above tag FAIL!");
                return SAVE_TAG_ERROR_DATABASE_ERROR;
            }
        } else {
            if (BuildConfig.DEBUG) {
                if (BuildConfig.DEBUG) Log.d("BillWiz", "Save the above tag SUCCESSFULLY!");
            }
            TAGS.add(tag);
...
// delete a coCoinRecord//////////////////////////////////////////////////////////////////////////////////
    public static long deleteRecord(final BillWizRecord billWizRecord, boolean deleteInList) {
        long deletedNumber = db.deleteRecord(billWizRecord.getId());
        if (deletedNumber > 0) {
            if (BuildConfig.DEBUG) Log.d("BillWiz",
                    "recordManager.deleteRecord: Delete " + billWizRecord.toString() + " S");
...
//                    .showToast(R.string.delete_successfully_locale, SuperToast.Background.BLUE);
            ...
            if (deleteInList) {
                ...
                for (int i = 0; i < RECORDS.size(); i++) {
                    if (RECORDS.get(i).getId() == billWizRecord.getId()) {
                        RECORDS.remove(i);
                        if (BuildConfig.DEBUG) Log.d("BillWiz",
                                "recordManager.deleteRecord: Delete in RECORD " + billWizRecord.toString() + " S");
                        break;
                    }
                }
            }
            ...
            if (BuildConfig.DEBUG) Log.d("BillWiz",
                    "recordManager.deleteRecord: Delete " + billWizRecord.toString() + " F");
//            BillWizToast.getInstance()
//                    .showToast(R.string.delete_failed_locale, SuperToast.Background.RED);
    ...
    public static int deleteTag(int id) {
        ...
        for (BillWizRecord coCoinRecord : RECORDS) {
        ...
        if (deletedId == -1) {
            if (BuildConfig.DEBUG) Log.d("BillWiz", "Delete the above tag FAIL!");
            return DELETE_TAG_ERROR_DATABASE_ERROR;
        } else {
            if (BuildConfig.DEBUG) Log.d("BillWiz", "Delete the above tag SUCCESSFULLY!");
            for (Tag tag : TAGS) {
    ...
    public static long updateRecord(final BillWizRecord billWizRecord) {
        long updateNumber = db.updateRecord(billWizRecord);
        if (updateNumber <= 0) {
            if (BuildConfig.DEBUG) {
                if (BuildConfig.DEBUG) Log.d("CoCoin", "recordManager.updateRecord " + billWizRecord.toString() + " F");
            }
//            BillWizToast.getInstance().showToast(R.string.update_failed_locale, SuperToast.Background.RED);
        } else {
            if (BuildConfig.DEBUG) {
                if (BuildConfig.DEBUG) Log.d("CoCoin", "recordManager.updateRecord " + billWizRecord.toString() + " S");
            }
            p = RECORDS.size() - 1;
            for (; p >= 0; p--) {
                if (RECORDS.get(p).getId() == billWizRecord.getId()) {
                    SUM -= (int)RECORDS.get(p).getMoney();
                    SUM += (int) billWizRecord.getMoney();
                    RECORDS.get(p).set(billWizRecord);
                    break;
                }
            }
            billWizRecord.setIsUploaded(false);
...
//            }
            db.updateRecord(billWizRecord);
//            BillWizToast.getInstance().showToast(R.string.update_successfully_locale, SuperToast.Background.BLUE);
        }
        return updateNumber;
    }
...
// after updating, get the old records from server//////////////////////////////////////////////////
                        ...
                        counter++;
                        coCoinRecord.setUserId(user.getObjectId());
                        coCoinRecord.save(BillWizApplication.getAppContext(), new SaveListener() {
                            @Override
                            public void onSuccess() {
                                if (BuildConfig.DEBUG) {
...
//get records from server to local//////////////////////////////////////////////////////////////////
    ...
    public static long getRecordsFromServer() {
        updateNum = 0;
        BmobQuery<BillWizRecord> query = new BmobQuery<BillWizRecord>();
        query.addWhereEqualTo("userId",
                BmobUser.getCurrentUser(BillWizApplication.getAppContext(), User.class).getObjectId());
        query.setLimit(Integer.MAX_VALUE);
        query.findObjects(BillWizApplication.getAppContext(), new FindListener<BillWizRecord>() {
            @Override
            public void onSuccess(List<BillWizRecord> object) {
                if (BuildConfig.DEBUG) {
                ...
                for (BillWizRecord coCoinRecord : object) {
                    boolean exist = false;
                    ...
                    if (!exist) {
                        BillWizRecord newCoCoinRecord = new BillWizRecord();
                ...
                Collections.sort(RECORDS, new Comparator<BillWizRecord>() {
                    @Override
                    public int compare(BillWizRecord lhs, BillWizRecord rhs) {
    ...
    public static int getCurrentMonthExpense() {
        ...
        Calendar left = BillWizUtil.GetThisMonthLeftRange(calendar);
        int monthSum = 0;
        for (int i = RECORDS.size() - 1; i >= 0; i--) {
    ...
    public static List<BillWizRecord> queryRecordByTime(Calendar c1, Calendar c2) {
        List<BillWizRecord> list = new LinkedList<>();
        for (BillWizRecord billWizRecord : RECORDS) {
            if (billWizRecord.isInTime(c1, c2)) {
    ...
    public static List<BillWizRecord> queryRecordByCurrency(String currency) {
        List<BillWizRecord> list = new LinkedList<>();
        for (BillWizRecord billWizRecord : RECORDS) {
            if (billWizRecord.getCurrency().equals(currency)) {
                list.add(billWizRecord);
            }
        }
        return list;
    ...
    public static List<BillWizRecord> queryRecordByTag(int tag) {
        List<BillWizRecord> list = new LinkedList<>();
        for (BillWizRecord billWizRecord : RECORDS) {
            if (billWizRecord.getTag() == tag) {
                list.add(billWizRecord);
            }
        }
        return list;
    ...
    public static List<BillWizRecord> queryRecordByMoney(double money1, double money2, String currency) {
        List<BillWizRecord> list = new LinkedList<>();
        for (BillWizRecord billWizRecord : RECORDS) {
            if (billWizRecord.isInMoney(money1, money2, currency)) {
                list.add(billWizRecord);
            }
        }
        return list;
    ...
    public static List<BillWizRecord> queryRecordByRemark(String remark) {
        List<BillWizRecord> list = new LinkedList<>();
        for (BillWizRecord billWizRecord : RECORDS) {
            if (BillWizUtil.IsStringRelation(billWizRecord.getRemark(), remark)) {
                list.add(billWizRecord);
            }
        }
        return list;
    ...
    private void randomDataCreater() {
        ...
        List<BillWizRecord> createdCoCoinRecords = new ArrayList<>();
        ...
        Collections.sort(createdCoCoinRecords, new Comparator<BillWizRecord>() {
            @Override
            public int compare(BillWizRecord lhs, BillWizRecord rhs) {
                if (lhs.getCalendar().before(rhs.getCalendar())) {
                    return -1;
        ...
        for (BillWizRecord billWizRecord : createdCoCoinRecords) {
            saveRecord(billWizRecord);
        }
    }
