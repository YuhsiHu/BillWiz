...
public class TagViewRecyclerViewAdapter
    ...
    private List<List<BillWizRecord>> contents;
    ...
    public TagViewRecyclerViewAdapter(List<BillWizRecord> coCoinRecords, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            Collections.sort(coCoinRecords, new Comparator<BillWizRecord>() {
                @Override
            ...
            double monthSum = 0;
            double yearSum = 0;
            List<BillWizRecord> yearSet = new ArrayList<>();
            List<BillWizRecord> monthSet = new ArrayList<>();

            for (BillWizRecord coCoinRecord : coCoinRecords) {
            ...
            if (chartType == PIE) {
                ...
                for (int i = 0; i < contents.size(); i++) {
                    ...
                    for (BillWizRecord coCoinRecord : contents.get(i)) {
                ...
                if (IS_EMPTY) {
                    ...
                    holder.from.setText(
                            mContext.getResources().getString(R.string.from) + " " +
                                    startYear + " " + BillWizUtil.GetMonthShort(startMonth));
                    holder.sum.setText(BillWizUtil.GetInMoney((int)Sum));
                    holder.to.setText(
                            mContext.getResources().getString(R.string.to) + " " +
                                    endYear + " " + BillWizUtil.GetMonthShort(endMonth));
                    holder.to.setTypeface(BillWizUtil.GetTypeface());
                    holder.from.setTypeface(BillWizUtil.GetTypeface());
                }
                holder.sum.setTypeface(BillWizUtil.typefaceLatoLight);
                break;
            case TYPE_CELL:
                ...
                holder.expanse.setTypeface(BillWizUtil.GetTypeface());
                switch (chartType) {
                    case PIE:
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_MONTH)) {
                            holder.date.setText(year + " " + BillWizUtil.GetMonthShort(month));
                        } else {
                            holder.date.setText(year + " ");
                        }

                        holder.expanse.setText(BillWizUtil.GetInMoney((int) (double) SumList.get(position - 1)));
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_YEAR)) {
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                ...
                                SubcolumnValue value = new SubcolumnValue(
                                        ...
                                        BillWizUtil.GetRandomColor());
                                value.setLabel(BillWizUtil.MONTHS_SHORT[month] + " " + year);
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                axisValueList.add(new AxisValue(i)
                                        .setLabel(BillWizUtil.GetMonthShort(i + 1)));
                            }
                            ...
                            for (int i = 0; i < numColumns; ++i) {
                                ...
                                SubcolumnValue value = new SubcolumnValue((float)
                                        DayExpanseSum[(year - startYear) * 372
                                                + (month - 1) * 31 + i],
                                        BillWizUtil.GetRandomColor());
                                value.setLabel(BillWizUtil.MONTHS_SHORT[month] + " " + (i + 1) + " " + year);
                                subcolumnValues.add(value);
                                Column column = new Column(subcolumnValues);
                                column.setHasLabels(false);
                                column.setHasLabelsOnlyForSelected(false);
        ...
        public void onValueSelected(final int columnIndex, int subColumnIndex, SubcolumnValue value) {
            Snackbar snackbar =
                    Snackbar.with(mContext)
                            ...
                            .margin(15, 15)
                            .backgroundDrawable(BillWizUtil.GetSnackBarBackground(fragmentTagId))
                            .textColor(Color.WHITE)
                            .textTypeface(BillWizUtil.GetTypeface())
                            .actionLabel(mContext.getResources().getString(R.string.check))
                            .actionLabelTypeface(BillWizUtil.typefaceLatoLight)
            ...
            if (fragmentPosition == SUM_HISTOGRAM) {
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    timeString = " " + BillWizUtil.GetMonthShort(month)
                            + " " + (columnIndex + 1) + " "
                    ...
                    if ("zh".equals(BillWizUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString + "\n" +
                                BillWizUtil.GetSpendString((int) value.getValue());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                BillWizUtil.GetSpendString((int) value.getValue());
                    } else {
                        text = BillWizUtil.GetSpendString((int) value.getValue()) + "\n" +
                                mContext.getResources().getString(R.string.on) + timeString;
                        dialogTitle = BillWizUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString;
                    }
                    snackbar.text(text);
                    snackbar.actionListener(new ActionClickListener() {
                        ...
                        public void onActionClicked(Snackbar snackbar) {
                            List<BillWizRecord> shownCoCoinRecords = new ArrayList<>();
                            boolean isSamed = false;
                            for (BillWizRecord coCoinRecord : contents.get(position)) {
                ...
                if (type.get(position).equals(SHOW_IN_YEAR)) {
                    ...
                    timeString = " " + BillWizUtil.GetMonthShort(columnIndex + 1) + " "
                            + timeString.substring(timeString.length() - 4, timeString.length());
                    if ("zh".equals(BillWizUtil.GetLanguage())) {
                ...
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    if ("zh".equals(BillWizUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString +
                                BillWizUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + BillWizUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                BillWizUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + BillWizUtil.GetTagName(contents.get(position).get(0).getTag());
                    } else {
                        text = BillWizUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + BillWizUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = BillWizUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + BillWizUtil.GetTagName(contents.get(position).get(0).getTag());
                    }
    ...
    private class PieValueTouchListener implements PieChartOnValueSelectListener {
        ...
        public void onValueSelected(int i, SliceValue sliceValue) {
            ...
            if ("zh".equals(BillWizUtil.GetLanguage())) {
                ...
                text = BillWizUtil.GetSpendString((int) sliceValue.getValue()) +
                        BillWizUtil.GetPercentString(percent) + "\n" +
                        "in " + BillWizUtil.GetTagName(RecordManager.TAGS.get(tagId).getId());
            ...
            Snackbar snackbar =
                    Snackbar
                            ...
                            .actionListener(new ActionClickListener() {
                                ...
                                public void onActionClicked(Snackbar snackbar) {
                                    List<BillWizRecord> shownCoCoinRecords = new ArrayList<>();
                                    for (BillWizRecord record : contents.get(position)) {
