...
import com.billwiz.admin.billwiz.model.Feedback;
...
public class HelpFeedbackFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        input.setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        help = (TextView)view.findViewById(R.id.helper);
        help.setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        number = (TextView)view.findViewById(R.id.number);
        number.setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (exceed) {
                    ...
                    BillWizUtil.getInstance().showToast(BillWizApplication.getAppContext(), BillWizApplication.getAppContext().getResources().getString(R.string.help_feedback_sent));
                    Feedback feedback = new Feedback();
                    feedback.setContent(input.getText().toString());
                    feedback.save(BillWizApplication.getAppContext(), new SaveListener() {
                        @Override
                        public void onSuccess() {
                            BillWizUtil.getInstance().showToast(BillWizApplication.getAppContext(), BillWizApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_successfully));
                        }

                        @Override
                        public void onFailure(int code, String arg0) {
                            BillWizUtil.getInstance().showToast(BillWizApplication.getAppContext(), BillWizApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_fail));
                        }
                    });
                }
            }
    ...
    private void setNumberText() {
        ...
        if (chineseIsDoubleCount) {
            count = BillWizUtil.getInstance().textCounter(input.getText().toString());
        } else {
