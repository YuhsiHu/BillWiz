...
public class BillWizRecord extends BmobObject {
    ...
    public BillWizRecord() {
        this.id = Long.valueOf(-1);
    }

    public BillWizRecord(long id, float money, String currency, int tag, Calendar calendar) {
        this.id = id;
        this.money = money;
    ...
    public BillWizRecord(long id, float money, String currency, int tag, Calendar calendar, String remark) {
        this.id = id;
        this.money = money;
        this.currency = currency;
        this.tag = tag;
    ...
    public void set(BillWizRecord coCoinRecord) {
    ...
    public boolean isInMoney(double money1, double money2, String currency) {
        return BillWizUtil.ToDollas(money1, currency) <= BillWizUtil.ToDollas(this.money, this.currency)
                && BillWizUtil.ToDollas(money2, currency) > BillWizUtil.ToDollas(this.money, this.currency);
    }

    public String getCalendarString() {
        return String.format("%02d", calendar.get(Calendar.HOUR_OF_DAY)) + ":"
                + String.format("%02d", calendar.get(Calendar.MINUTE)) + " "
                + BillWizUtil.GetMonthShort(calendar.get(Calendar.MONTH) + 1) + " "
