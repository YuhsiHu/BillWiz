...
import com.billwiz.admin.billwiz.adapter.PasswordChangeButtonGridViewAdapter;
import com.billwiz.admin.billwiz.adapter.PasswordChangeFragmentAdapter;
...
//    private SuperToast superToast;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int iconOffsetX = BillWizUtil.getInstance().dpToPx(28);
        int iconOffsetY = BillWizUtil.getInstance().dpToPx(28);
        ...
        ((TextView)findViewById(R.id.text_0)).setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        ...
        for (int i = 0; i < 5; ++i) {
            SliceValue sliceValue = new SliceValue((float) Math.random() * 30 + 15, ContextCompat.getColor(BillWizApplication.getAppContext(), R.color.white));
        ...
        for (int i = 0; i < 1; ++i) {
            ...
            aLine.setColor(ContextCompat.getColor(BillWizApplication.getAppContext(), R.color.white));
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            for (int j = 0; j < 1; ++j) {
                subcolumnValues.add(new SubcolumnValue((float) Math.random() * 50f + 5, ContextCompat.getColor(BillWizApplication.getAppContext(), R.color.white)));
            }
        ...
        sc7.startToPosition(size.x / 2 - BillWizUtil.getInstance().dpToPx(140), size.y * 8 / 9 - BillWizUtil.getInstance().dpToPx(140) + size.y);
        ...
        ((TextView)findViewById(R.id.text_1)).setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        ...
        sc9.startToPosition(size.x / 2 - BillWizUtil.getInstance().dpToPx(100) + size.x, size.y / 7);
        ...
        sc10.startToPosition(size.x / 2 - size.x, size.y * 6 / 7 - BillWizUtil.getInstance().dpToPx(100));
        ...
        ((TextView)findViewById(R.id.text_2)).setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
        ...
        ((TextView)findViewById(R.id.text_3)).setTypeface(BillWizUtil.getInstance().typefaceLatoLight);
                ...
                new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        viewPagerLayoutParams.topMargin
                                = getStatusBarHeight() + BillWizUtil.getToolBarHeight(mContext) / 2;
...
//        SuperToast.cancelAllSuperToasts();
        ...
        switch (CURRENT_STATE) {
            case NEW_PASSWORD:
                if (BillWizUtil.ClickButtonDelete(position)) {
                    if (longClick) {
                        BillWizFragmentManager.passwordChangeFragment[CURRENT_STATE].init();
                        newPassword = "";
                    } else {
                        BillWizFragmentManager.passwordChangeFragment[CURRENT_STATE]
                    ...
                    newPassword += BillWizUtil.BUTTONS[position];
            ...
            case PASSWORD_AGAIN:
                if (BillWizUtil.ClickButtonDelete(position)) {
                    ...
                    againPassword += BillWizUtil.BUTTONS[position];
                        ...
                        if (againPassword.equals(newPassword)) {
                            ...
                            if (SettingManager.getInstance().getLoggenOn()) {
                                User currentUser = BmobUser.getCurrentUser(
                                        BillWizApplication.getAppContext(), User.class);
                            ...
                            BillWizFragmentManager.passwordChangeFragment[CURRENT_STATE].clear(4);
                            BillWizFragmentManager.passwordChangeFragment[CURRENT_STATE - 1].init();
    ...
    private void showToast(int toastType) {
        ...
        switch (toastType) {
            // old password wrong
            case 0:
                Toasty.error(mContext, mContext.getResources().getString(R.string.toast_password_wrong)).show();
                break;
            // password is different
            case 1:
                Toasty.warning(mContext, mContext.getResources().getString(R.string.different_password)).show();
                break;
            // success
            case 2:
                Toasty.success(mContext, mContext.getResources().getString(R.string.set_password_successfully)).show();
                break;
            default:
                break;
        }
    ...
    protected void onDestroy() {
        for (int i = 0; i < 3; i++) {
            BillWizFragmentManager.passwordChangeFragment[i].onDestroy();
            BillWizFragmentManager.passwordChangeFragment[i] = null;
        }
        super.onDestroy();
    }
